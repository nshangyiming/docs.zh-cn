...
|storage_usage_soft_limit_percent|90|BE 存储目录下空间使用率的最大值。如果超限，则不能继续往该路径 clone tablet。|
|storage_usage_soft_limit_reserve_bytes|200 \* 1024 \* 1024 \* 1024|BE 存储目录下剩余空间的最小值，单位为 Byte。如果超限，则不能继续往该路径 clone tablet。|
...
| storage_flood_stage_left_capacity_bytes               | 107374182400| Byte   | 如果剩余空间小于该值，会拒绝 Load 和 Restore 作业，默认 100GB。 |
